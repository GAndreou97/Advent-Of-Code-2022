    ...
    public static class Day4{
        ...
        public static int part2(){
            ...
            foreach (string line in System.IO.File.ReadLines(@"/Users/georgeandreou/Documents/GitHub/Advent-Of-Code-2022/Day 4/Input.txt")){
                ...
                if (Int32.Parse(firstZones[0]) >= Int32.Parse(secondZones[0]) && Int32.Parse(firstZones[0]) <= Int32.Parse(secondZones[1]) ){
                    numOfOverlaps = numOfOverlaps +1;
                }
                else if (Int32.Parse(firstZones[1]) >= Int32.Parse(secondZones[0]) && Int32.Parse(firstZones[1]) <= Int32.Parse(secondZones[1]) ){
                    numOfOverlaps = numOfOverlaps +1;
                }
                else if (Int32.Parse(secondZones[0]) >= Int32.Parse(firstZones[0]) && Int32.Parse(secondZones[0]) <= Int32.Parse(firstZones[1]) ){
                    numOfOverlaps = numOfOverlaps +1;
                }
                else if (Int32.Parse(secondZones[1]) >= Int32.Parse(firstZones[0]) && Int32.Parse(secondZones[1]) <= Int32.Parse(firstZones[1]) ){
                    numOfOverlaps = numOfOverlaps +1;
                }
